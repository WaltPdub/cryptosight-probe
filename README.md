# CryptoSight On-Prem Probe
  ...
  **Optional** — restrict which IP ranges are scanned. Find the `scan:` section and add your networks.
  If you leave `networks` empty the probe will scan all reachable hosts on common TLS ports,
  which can take longer on large networks:
  ...
  ```
  probe | Starting active TLS scan on 192.168.1.0/24
  probe | Scan complete — 12 assets found
  probe | Sending 12 assets to CryptoSight
  probe | Done. Next run at 02:00.
  ```
  ...
  scan:
    networks: []                    # CIDR ranges to scan; [] = all reachable hosts
  ...
  sniffer:                          # only used when mode.passiveSniffer: true
    interface: "eth0"               # network interface to capture (use "any" for all)
  ...
  **Requires:** Linux host with Docker, `NET_RAW`/`NET_ADMIN` capabilities, `network_mode: host`.
  **Not available** on Docker Desktop for Mac or Windows (VM networking prevents host-level packet capture).
  ...
  **Helm:**
  ...
  | Sniffer not available | Docker Desktop (Mac/Windows) does not support host networking. Deploy on Linux for sniffer support. |
